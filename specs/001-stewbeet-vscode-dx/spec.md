...
- **FR-006**: Mapping emission MUST be opt-in, and when it is on, every artifact the build produces MUST carry the same content. A map that reaches `build/` but not the zip `copy_to_destination` ships is worse than no map, because the zip is what the game loads and a debugger looking beside the function would find nothing there.
...
- **FR-024**: A source file no vanilla parser can read MUST be removable from Spyglass's own view, from both sides of the boundary. A language id keeps Spyglass off the document being edited and not off the data pack it indexes from disk, and no API lets one extension clear another's diagnostics, so the project's `env.exclude` is the only mechanism. The editor MUST offer it for the file being opened, since a project that has never been built is exactly when this bites. The build MUST be able to maintain the same list without being asked twice: it knows which files bolt compiled and which ones mecha split, where the editor can only read the text and guess. Whichever writes it MUST leave every other key alone, MUST retract only patterns it added itself, and MUST write nothing when there is nobody to consent.
