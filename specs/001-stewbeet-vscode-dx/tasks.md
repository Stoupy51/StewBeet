...
- [X] T004 Extract the dialect-free half of `python_package/stewbeet/plugins/sniffer/emit.py` into `python_package/stewbeet/plugins/sniffer/sidecar.py`: `SOURCE_MAPPING_URL`, `function_file_path`, `source_root_for`, `pack_output_depth`, `carries_discovery_comment`, and the append-and-write body of `write_maps` as a `write_sidecar(ctx, path, func, mapped)` taking already-resolved `dict[int, SourceOrigin]`. What stays in `emit.py` is the one StewBeet-specific line, reading `Mem.source_map_chunks` and running `align`. Carry the doctests across unchanged
- [X] T005 Move the project-source filter out of `python_package/stewbeet/plugins/sniffer/origin.py` into the shared half, so FR-010 is enforced by one gate for both producers. Only the filter moves: the frame walk, the AST index and the tier ordering are StewBeet's reconstruction machinery and have no meaning for a producer whose positions were never lost
- [X] T006 Proved step B is untouched by the split: `plugin_22_sniffer_source_maps`, `plugin_23_sniffer_with_headers` and `plugin_24_sniffer_attribution` all pass. Their assertion plugins are the evidence, not a clean `git status`: `python_package/tests/.gitignore` ignores `build/`, so no emitted map is tracked and a clean tree says nothing about them. What the assertions do check is every part a refactor could break: the `sourceRoot` arithmetic, that `sourceRoot` plus `sources` resolves to a real file, that the first mapping lands on a `write_function` call, and that the zip carries what the build directory does
...
- [X] T018 [US2] Create `python_package/stewbeet/plugins/sniffer/mecha/__init__.py` with a `beet_default(ctx)` generator doing its work after `yield`, and say in its docstring that it must be listed **before** `mecha` in the pipeline. Listed after, the `Module` compilation units are gone from `mc.database` by the time it runs, and their sources are what T019 needs. This is the opposite of `stewbeet.plugins.sniffer.emit`'s placement rule and will be got wrong otherwise
- [X] T019 [US2] Implement `owner_of(location, sources)` in `python_package/stewbeet/plugins/sniffer/mecha/attribute.py`: for each candidate compilation unit that has a `source`, accept it when offset `location.pos` sits at line `location.lineno` column `location.colno` of that source. Candidates are every unit in `mc.database` carrying a source, which is what makes the check work across modules. Doctest it on the probe's own numbers
- [X] T020 [US2] Return `None` from `owner_of` when zero or more than one candidate matches, in `python_package/stewbeet/plugins/sniffer/mecha/attribute.py`, and emit that line unmapped. FR-010 is a validity condition, not a quality target: a jump to the wrong file is worse than no jump, and a two-line module can easily agree with a longer one by accident
- [X] T021 [US2] Reject candidates that fail the shared project-source filter from T005, in `python_package/stewbeet/plugins/sniffer/mecha/attribute.py`. shulker loads `vendor/*`, so a bolt project maps into installed libraries by default unless this is applied, and FR-010 forbids that as firmly for bolt as for StewBeet
- [X] T022 [US2] Reconcile the serialised commands against the function's final text with the existing `align` in `python_package/stewbeet/plugins/sniffer/align.py`, one `WriteChunk` per command, rather than trusting positions. **Changed during implementation**: this task and T023 were written to count lines and bail on a mismatch, which the minimal template disproved. That template runs `stewbeet.plugins.auto.headers.silent` after `mecha`, so every function gains six lines after compilation and a bail would have emitted nothing at all. `align` already reconciles exactly that for the StewBeet producer, so the mecha producer reuses it and the counting module was deleted
- [X] T023 [US2] Cover the disagreement cases through `align` rather than a bail, in `python_package/tests/plugin_25_sniffer_mecha_bolt/src/assertions.py`: a header block prepended after mecha stays unmapped and every command keeps its origin through the shift. Folded into T022, which explains why. A `difflib` reconciliation degrades per line where a bail degrades per function, which is strictly better and is what the StewBeet side already does
- [X] T024 [US2] Convert mecha's 1-based `lineno` and `colno` to the map's 0-based fields in `python_package/stewbeet/plugins/sniffer/mecha/__init__.py`, and keep the real column rather than zeroing it. Column precision is the one thing this producer gets for free and the StewBeet path explicitly does not promise, so throwing it away here would be a choice, not a limitation
- [X] T025 [US2] Write the sidecars through `write_sidecar` from T004, in `python_package/stewbeet/plugins/sniffer/mecha/__init__.py`, so the discovery comment, the `sourceRoot` arithmetic and the idempotence check are the ones step B already validated against Sniffer's reference implementation
- [X] T026 [US2] [P] Add `python_package/tests/plugin_25_sniffer_mecha_bolt/` as a beet project: two `.bolt` modules both appending to one function, the emitter before `mecha` in the pipeline, and an assertions plugin that decodes the emitted `mappings` and asserts each line's source and line number. This is the probe promoted to a regression test, and the multi-module shape is the assertion that matters
- [X] T027 [P] [US2] Cover a `.mcfunction` compiled by mecha with no bolt module involved, in `python_package/tests/plugin_25_sniffer_mecha_bolt/`. The minimal template's own `hello.mcfunction` is that case and more: a `for` loop turns one source line into five commands, and `execute function ./goodbye:` nests two further functions out of the same file. FR-014 says the format and encoder are shared across dialects, and a file that is not a `.bolt` module at all is the cheapest proof the emitter is not secretly bolt-specific
- [X] T028 [US2] Assert in the same folder that a module under the loaded `vendor/` tree produces unmapped lines rather than a mapping into it, per T021. The rule is only worth anything if a test would notice it being dropped
...
- [X] T029 [US3] Built [shulker](file:///d:/advanced_desktop/shulker) with the emitter in its own `uv` environment, StewBeet added with `uv run --with-editable`. **381 of 777 functions got a map, and 815 of 2802 command lines are mapped.** The rest unmapped is the rule working rather than failing: of the misses, 419 come from modules bolt synthesises in memory with no file at all, 513 carry a position valid in no project source, and 221 have a line and column that are valid in the declaring file while the offset disagrees. That last group is the dangerous one, and sampling it showed why the offset check must stay: `advancement revoke @s only item:testing_using/...` would have been mapped onto the line `item_name = "Component Test: Using"`, which is exactly the plausible-looking wrong jump FR-010 forbids
- [X] T030 [US3] Ran `extension/vscode/src/sourcemap.js` unchanged over the 381 bolt-produced maps: **815 of 815 origins resolve to a real `.bolt` file at a real line**, and `git diff extension/vscode/src/` is empty, so step E needed no JavaScript at all. `event/reset.mcfunction` resolves into three different modules, `net.bolt`, `rage_juice.bolt` and `cutlass.bolt`, which is the multi-module case on a real project. What this does not cover is the click itself in a running editor, since the provider needs a live VS Code; the module every provider calls is what was exercised
- [ ] T031 [US3] Set a Sniffer breakpoint from a `.bolt` line against the same build, the way [quickstart.md](./quickstart.md)'s interop section does it for StewBeet. **Not done, and not doable from here**: it needs a running Minecraft client with the Sniffer mod attached, which no local tooling substitutes for. The map format is byte-identical to the one plugin_22 already validates against Sniffer's own reference implementation, so the risk is low, but SC-002 stays unproven for bolt until someone runs it
...
- [X] T032 Measured against NFR-002 on shulker, median of five builds each: **44.99s baseline against 48.95s with the emitter, 8.8% overhead** against a 20% budget. The fixture project was too small to measure, since interpreter startup dominates a four-function build
- [X] T033 [P] Update the sequencing and layers tables in [contracts/dialects.md](./contracts/dialects.md) so D and E read as shipped, and add the emitter's pipeline placement rule to the "Emitting maps per dialect" section. That section currently describes the mecha producer as a plan
- [X] T034 [P] Add a "two producers, one format" section to [contracts/source-map.md](./contracts/source-map.md) stating what differs between them: the mecha side carries real columns, the StewBeet side maps to a line and column zero. A consumer reading both should know which promise it is holding
- [X] T035 [P] Add phases D and E to [quickstart.md](./quickstart.md) with their validation steps, including the `mecha.contrib.source_map` name collision, so the next reader does not require it expecting a source map and get a header comment
- [X] T036 [P] Update `extension/vscode/README.md` and `extension/vscode/CHANGELOG.md` for the bolt language id, and bump `extension/vscode/package.json`. The README currently describes a Python-only extension, and this is the first thing it does for a file that is not `.py`
- [X] T037 [P] Record the `languages` contribution and the `source.bolt` grammar in [contracts/extension-api.md](./contracts/extension-api.md), whose "Grammar: what is highlighted as mcfunction" section covers only the Python injection
- [X] T038 [P] Move FR-016 and FR-017 out of the deferred block in [spec.md](./spec.md), deleting the sentence that says neither is in the current pass and both get their own `/speckit-tasks` list when their turn comes. Their turn is this list
- [X] T039 Rebuild the extension package with `extension/build_publish.sh`, which copies the icon and licence in and cleans them up afterwards. Do not call `vsce` directly
- [X] T040 Run the full gate: `npm test` and `npm run test:integration` in `extension/vscode/`, plus `ruff check stewbeet tests`, `pyright stewbeet tests`, `scripts/sync_api.py --check`, `scripts/all_doctests.py` and `scripts/run_integration_tests.py` in `python_package/`
