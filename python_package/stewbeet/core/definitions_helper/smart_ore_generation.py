...
class CustomOreGeneration:
	...
	OVERWORLD_REPLACEABLES: tuple[str, ...] = (
	...
	def __init__(
		...
		Args:
			...
			provider			(str|list[str]):	The block or block tag to check for the ore to generate, can be either a string or a list of strings
				(default: the blocks vanilla carvers replace in the overworld, see OVERWORLD_REPLACEABLES)
		...
		"""
		if provider is None:
			provider = list(self.OVERWORLD_REPLACEABLES) if minecraft_version_at_least((26, 3)) else "#minecraft:overworld_carver_replaceables"
